...
import { AppBar, Avatar, Badge, Box, Breadcrumbs, Divider, Drawer, IconButton, InputAdornment, List, ListItemButton, ListItemIcon, ListItemText, ListSubheader, Menu, MenuItem, Stack, TextField, Toolbar, Tooltip, Typography, useMediaQuery, useTheme } from "@mui/material";
...
export default function DashboardLayout() {
  ...
  const page = useMemo(() => MENU.flatMap((group) => group.items).find((item) => item.to === location.pathname) || { label: "DL8" }, [location.pathname]);

  const navigation = (compact = false) => (
    <Box sx={{ display: "flex", flexDirection: "column", height: "100%" }}>
      <Stack direction="row" alignItems="center" spacing={1.25} sx={{ minHeight: 72, px: compact ? 2.25 : 2.5 }}>
        <Box sx={{ width: 34, height: 34, display: "grid", placeItems: "center", borderRadius: 2, bgcolor: "primary.main", color: "primary.contrastText" }}><BakeryDiningIcon fontSize="small" /></Box>
        {!compact && <Box><Typography variant="subtitle1" sx={{ lineHeight: 1.15 }}>DL8 ERP</Typography><Typography variant="caption" color="text.secondary">Gestión operativa</Typography></Box>}
      </Stack>
      <Divider />
      <Box sx={{ flex: 1, overflowY: "auto", px: 1.25, py: 1.5 }}>
        {MENU.map((group) => (
          <List key={group.section} disablePadding subheader={!compact && <ListSubheader disableSticky sx={{ bgcolor: "transparent", px: 1.25, lineHeight: "30px", fontSize: 10.5, fontWeight: 700, letterSpacing: ".07em", textTransform: "uppercase" }}>{group.section}</ListSubheader>}>
            {group.items.filter((item) => !item.roles || hasRole(...item.roles)).map((item) => {
              const selected = item.to === location.pathname;
              const content = <ListItemButton selected={selected} onClick={() => { navigate(item.to); setMobileOpen(false); }} sx={{ justifyContent: compact ? "center" : "flex-start", px: compact ? 0 : 1.25 }}><ListItemIcon sx={{ minWidth: compact ? 0 : 38, color: "text.secondary", justifyContent: "center" }}>{item.icon}</ListItemIcon>{!compact && <ListItemText primary={item.label} primaryTypographyProps={{ fontSize: 14, fontWeight: selected ? 650 : 500 }} />}</ListItemButton>;
              return compact ? <Tooltip key={item.to} title={item.label} placement="right">{content}</Tooltip> : <Box key={item.to}>{content}</Box>;
            })}
          </List>
        ))}
      </Box>
  ...
  return (
    <Box sx={{ display: "flex", minHeight: "100vh" }}>
      <Box component="nav" sx={{ width: { md: drawerWidth }, flexShrink: { md: 0 }, transition: "width .2s ease" }}>
        <Drawer variant="temporary" open={mobileOpen} onClose={() => setMobileOpen(false)} ModalProps={{ keepMounted: true }} sx={{ display: { xs: "block", md: "none" }, "& .MuiDrawer-paper": { width: DRAWER_WIDTH } }}>{navigation(false)}</Drawer>
        <Drawer variant="permanent" open sx={{ display: { xs: "none", md: "block" }, "& .MuiDrawer-paper": { width: drawerWidth, boxSizing: "border-box", transition: "width .2s ease" } }}>{navigation(collapsed)}</Drawer>
      </Box>
      <Box component="main" sx={{ flexGrow: 1, width: { md: `calc(100% - ${drawerWidth}px)` }, minWidth: 0, bgcolor: "background.default" }}>
        <AppBar position="sticky"><Toolbar sx={{ minHeight: "72px !important", px: { xs: 2, sm: 3 }, gap: 1.5 }}><IconButton onClick={() => mobile ? setMobileOpen(true) : setCollapsed((value) => !value)} sx={{ display: { md: "none" } }}><MenuIcon /></IconButton><Box sx={{ display: { xs: "none", sm: "block" }, flex: 1 }}><Breadcrumbs separator={<ChevronRightIcon fontSize="small" />} aria-label="breadcrumb"><Typography variant="caption" color="text.secondary">Inicio</Typography><Typography variant="caption" color="text.primary">{page.label}</Typography></Breadcrumbs></Box><TextField placeholder="Buscar en el ERP…" size="small" sx={{ width: { xs: 44, sm: 260 }, "& .MuiOutlinedInput-root": { bgcolor: "background.default" } }} InputProps={{ startAdornment: <InputAdornment position="start"><SearchIcon fontSize="small" /></InputAdornment> }} /><Tooltip title={mode === "light" ? "Cambiar a modo oscuro" : "Cambiar a modo claro"}><IconButton onClick={toggleMode}>{mode === "light" ? <DarkModeIcon /> : <LightModeIcon />}</IconButton></Tooltip><Tooltip title="Notificaciones"><IconButton><Badge color="error" variant="dot" invisible><NotificationsIcon /></Badge></IconButton></Tooltip><Tooltip title="Cuenta"><IconButton onClick={(event) => setAccountAnchor(event.currentTarget)} sx={{ p: 0.25 }}><Avatar sx={{ width: 34, height: 34, fontSize: 13, bgcolor: "primary.main" }}>{usuario?.nombre?.[0]?.toUpperCase() || "?"}</Avatar></IconButton></Tooltip><Menu anchorEl={accountAnchor} open={Boolean(accountAnchor)} onClose={() => setAccountAnchor(null)} PaperProps={{ sx: { minWidth: 210, mt: 1 } }}><Box sx={{ px: 2, py: 1.25 }}><Typography variant="subtitle2">{usuario?.nombre || "Usuario"}</Typography><Typography variant="caption" color="text.secondary" sx={{ textTransform: "capitalize" }}>{usuario?.rol}</Typography></Box><Divider /><MenuItem onClick={() => { logout(); navigate("/login"); }}><LogoutIcon fontSize="small" sx={{ mr: 1.5 }} />Cerrar sesión</MenuItem></Menu></Toolbar></AppBar>
        <Box sx={{ px: { xs: 2, sm: 3, lg: 4 }, py: { xs: 2.5, sm: 3.5 }, maxWidth: 1680, mx: "auto" }}><Outlet /></Box>
      </Box>
    </Box>
  );
}
