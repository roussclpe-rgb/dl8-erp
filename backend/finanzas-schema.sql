...
-- Los costos fijos pertenecen a un producto dentro de una versión de política.
-- Se mantiene la columna histórica de mpf_politicas para poder leer políticas antiguas.
CREATE TABLE IF NOT EXISTS mpf_politica_costos_fijos (id INTEGER PRIMARY KEY AUTOINCREMENT, politica_id INTEGER NOT NULL REFERENCES mpf_politicas(id) ON DELETE CASCADE, receta_grupo_id INTEGER NOT NULL, costo_unidad_minor INTEGER NOT NULL CHECK(costo_unidad_minor>=0), moneda TEXT NOT NULL DEFAULT 'PEN', UNIQUE(politica_id,receta_grupo_id));
...
CREATE TABLE IF NOT EXISTS mpf_reversiones_aplicacion (id INTEGER PRIMARY KEY AUTOINCREMENT, aplicacion_id_original INTEGER NOT NULL UNIQUE, evento_financiero_id INTEGER NOT NULL REFERENCES fin_eventos_financieros(id), entidad_id INTEGER NOT NULL REFERENCES fin_entidades_economicas(id), politica_id INTEGER NOT NULL REFERENCES mpf_politicas(id), politica_version INTEGER NOT NULL, motivo TEXT NOT NULL, revertido_por INTEGER NOT NULL REFERENCES usuarios(id), creado_en TEXT NOT NULL DEFAULT(datetime('now')), datos_reversion_json TEXT NOT NULL);
...
CREATE TABLE IF NOT EXISTS mpf_metas_financieras (id INTEGER PRIMARY KEY AUTOINCREMENT, entidad_id INTEGER NOT NULL REFERENCES fin_entidades_economicas(id), nombre TEXT NOT NULL, bolsillo_id INTEGER NOT NULL REFERENCES fin_bolsillos(id), monto_objetivo_minor INTEGER NOT NULL CHECK(monto_objetivo_minor>0), fecha_objetivo TEXT, tipo TEXT NOT NULL DEFAULT 'unica' CHECK(tipo IN ('unica','recurrente')), frecuencia_recurrencia TEXT CHECK(frecuencia_recurrencia IN ('semanal','mensual','trimestral','anual')), meta_origen_id INTEGER REFERENCES mpf_metas_financieras(id), saldo_inicial_minor INTEGER NOT NULL DEFAULT 0, estado TEXT NOT NULL DEFAULT 'activa' CHECK(estado IN ('activa','pausada','cumplida','cancelada')), cumplida_en TEXT, cumplida_evento_financiero_id INTEGER REFERENCES fin_eventos_financieros(id), cumplida_por INTEGER REFERENCES usuarios(id), creado_por INTEGER NOT NULL REFERENCES usuarios(id), creado_en TEXT NOT NULL DEFAULT(datetime('now')), actualizado_por INTEGER REFERENCES usuarios(id), actualizado_en TEXT);
...
CREATE TABLE IF NOT EXISTS fin_bolsillos (
  ...
  moneda TEXT NOT NULL DEFAULT 'PEN' CHECK (moneda = 'PEN'), descripcion TEXT, prioridad INTEGER NOT NULL DEFAULT 0 CHECK (prioridad >= 0), saldo_minimo_minor INTEGER NOT NULL DEFAULT 0 CHECK (saldo_minimo_minor >= 0), permite_saldo_negativo INTEGER NOT NULL DEFAULT 0 CHECK (permite_saldo_negativo IN (0,1)),
  estado TEXT NOT NULL DEFAULT 'activa' CHECK (estado IN ('activa','bloqueada','inactiva')),
  creado_por INTEGER NOT NULL REFERENCES usuarios(id), creado_en TEXT NOT NULL DEFAULT (datetime('now')),
  actualizado_por INTEGER NOT NULL REFERENCES usuarios(id), actualizado_en TEXT NOT NULL DEFAULT (datetime('now')),
  UNIQUE(entidad_id, codigo)
...
CREATE TABLE IF NOT EXISTS fin_auditoria (
  ...
  accion TEXT NOT NULL CHECK (accion IN ('crear','actualizar','bloquear','inactivar','cerrar_periodo','cambiar_acceso','cumplir_automaticamente','reabrir_por_reversion')),
...
END;
CREATE TRIGGER IF NOT EXISTS trg_fin_cobros_no_update BEFORE UPDATE OF entidad_id,pago_id,documento_cxc_id,evento_financiero_id,cuenta_financiera_id,bolsillo_id,turno_caja_id,metodo_pago,importe_minor,fecha,creado_por,creado_en ON fin_cobros BEGIN SELECT RAISE(ABORT,'Los datos de un cobro confirmado son inmutables'); END;
CREATE TRIGGER IF NOT EXISTS trg_fin_cobros_estado_update BEFORE UPDATE OF estado ON fin_cobros WHEN NOT (OLD.estado='confirmado' AND NEW.estado='revertido') BEGIN SELECT RAISE(ABORT,'La única transición válida del cobro es confirmado a revertido'); END;
...
CREATE TRIGGER IF NOT EXISTS trg_fin_aplicaciones_no_update BEFORE UPDATE OF documento_cxc_id,cobro_id,evento_financiero_id,importe_minor,fecha_aplicacion,creado_por,creado_en ON fin_aplicaciones_cxc BEGIN SELECT RAISE(ABORT,'Los datos de una aplicación CxC confirmada son inmutables'); END;
CREATE TRIGGER IF NOT EXISTS trg_fin_aplicaciones_estado_update BEFORE UPDATE OF estado ON fin_aplicaciones_cxc WHEN NOT (OLD.estado='confirmada' AND NEW.estado='revertida') BEGIN SELECT RAISE(ABORT,'La única transición válida de la aplicación es confirmada a revertida'); END;
