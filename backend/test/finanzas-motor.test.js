...
test('transferencia conserva total, mueve respaldo y queda auditada',()=>{const x=base('M2');motor.inicial(x.e.id,admin,'i2',{cuenta_financiera_id:x.c1.id,bolsillo_id:x.b,importe_minor:50000,fecha:'2026-07-01'});const transferencia=motor.transferencia(x.e.id,admin,'t2',{cuenta_origen_id:x.c1.id,cuenta_destino_id:x.c2.id,bolsillo_origen_id:x.b,importe_minor:20000,fecha:'2026-07-01'});assert.equal(motor.saldo(x.c1.id,x.b).tes,30000);assert.equal(motor.saldo(x.c2.id,x.b).tes,20000);assert.equal(motor.saldo(x.c2.id,x.b).bol,20000);assert.equal(db.prepare("SELECT COUNT(*) n FROM fin_auditoria WHERE entidad_tabla='fin_eventos_financieros' AND entidad_registro_id=?").get(transferencia.id).n,1);assert.equal(motor.utilidadPeriodoActual(x.e.id,new Date('2026-07-14')).utilidad_minor,0)});
